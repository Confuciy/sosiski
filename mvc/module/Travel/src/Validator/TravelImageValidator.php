...
class TravelImageValidator extends AbstractValidator
{
    ...
    {
        $this->fileAdapter = new FileInput('image');
...
//                imagedestroy($idest);
//
//                $res = new \Zend\Db\TableGateway\TableGateway('travels', $options['dbAdapter']);
//                $sql = $res->getSql();
//                $update = $sql->update();
//                $update->table('travels');
//                $update->set(['image' => $destination]);
//                $update->where(array('travel_id' => $options['travel_id']));
//                $statement = $sql->prepareStatementForSqlObject($update);
//                $statement->execute($sql);
//
//                return true;
